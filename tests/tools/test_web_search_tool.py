...
from mathclaw.agent.tools.web import WebSearchTool
from mathclaw.config.schema import WebSearchConfig
...
async def test_brave_search(monkeypatch):
    ...
    result = await tool.execute(query="mathclaw", count=1)
...
async def test_duckduckgo_search(monkeypatch):
    ...
    monkeypatch.setattr("mathclaw.agent.tools.web.DDGS", MockDDGS, raising=False)
    import mathclaw.agent.tools.web as web_mod
