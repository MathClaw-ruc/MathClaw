...
import mathclaw.channels.matrix as matrix_module
from mathclaw.bus.events import OutboundMessage
from mathclaw.bus.queue import MessageBus
from mathclaw.channels.matrix import (
    MATRIX_HTML_FORMAT,
    TYPING_NOTICE_TIMEOUT_MS,
    MatrixChannel,
)
from mathclaw.channels.matrix import MatrixConfig
...
async def test_start_skips_load_store_when_device_id_missing(
    ...
    monkeypatch.setattr("mathclaw.channels.matrix.get_data_dir", lambda: tmp_path)
    monkeypatch.setattr(
        "mathclaw.channels.matrix.AsyncClientConfig",
        lambda **kwargs: SimpleNamespace(**kwargs),
    )
    monkeypatch.setattr("mathclaw.channels.matrix.AsyncClient", _fake_client)
    monkeypatch.setattr(
        "mathclaw.channels.matrix.asyncio.create_task", _fake_create_task
    )
