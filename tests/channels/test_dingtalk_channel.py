...
try:
    from mathclaw.channels import dingtalk
...
import mathclaw.channels.dingtalk as dingtalk_module
from mathclaw.channels.dingtalk import DingTalkChannel, NanobotDingTalkHandler
from mathclaw.channels.dingtalk import DingTalkConfig
...
async def test_handler_processes_file_message(monkeypatch) -> None:
    ...
    async def fake_download(download_code, filename, sender_id):
        return f"/tmp/mathclaw_dingtalk/{sender_id}/{filename}"
    ...
    assert "/tmp/mathclaw_dingtalk/user1/report.xlsx" in msg.content
...
async def test_download_dingtalk_file(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "mathclaw.config.paths.get_media_dir",
