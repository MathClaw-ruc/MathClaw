"""Utility functions for mathclaw."""
...
def build_status_content(
    *,
    ...
    return "\n".join([
        f"\U0001f408 mathclaw v{version}",
...
def sync_workspace_templates(workspace: Path, silent: bool = False) -> list[str]:
    ...
    try:
        tpl = pkg_files("mathclaw") / "templates"
    except Exception:
        return []
    if not tpl.is_dir():
        return []
    ...
    _ensure_dir(workspace / "memory" / "daily_memory")
    _ensure_dir(workspace / "memory" / "weekly_memory")
    _ensure_dir(workspace / "memory" / "graphs")
