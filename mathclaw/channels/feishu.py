...
class FeishuChannel(BaseChannel):
    ...
    def __init__(self, config: Any, bus: MessageBus):
        ...
        self._reaction_enabled = bool(self.config.react_emoji)
        ...
        try:
            ...
            if not response.success():
                if response.code == 99991672:
                    self._reaction_enabled = False
                    logger.info("Feishu reaction disabled: missing permission ({})", response.msg)
                    return
    ...
    async def _add_reaction(self, message_id: str, emoji_type: str = "THUMBSUP") -> None:
        ...
        """
        if not self._client or not emoji_type or not self._reaction_enabled:
            return
        ...
        try:
            ...
            if not response.success():
                if response.code == 99991672:
                    ...
                    logger.info("Feishu streaming disabled: missing cardkit permission ({})", response.msg)
                    return None
    ...
    async def send_delta(self, chat_id: str, delta: str, metadata: dict[str, Any] | None = None) -> None:
        ...
        if not self._client or not self._streaming_enabled:
            return
        meta = metadata or {}
        loop = asyncio.get_running_loop()
        ...
        try:
            ...
            if msg.content and msg.content.strip():
                text_body = json.dumps(
                    {"text": render_plain_reply(msg.content.strip())},
                    ensure_ascii=False,
                )
