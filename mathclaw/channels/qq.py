...
from ..utils.text_render import render_plain_reply

try:
    from ..config.paths import get_media_dir
...
_QQ_TABLE_RE = re.compile(
    r"((?:^[ \t]*\|.+\|[ \t]*\n)(?:^[ \t]*\|[-:\s|]+\|[ \t]*\n)(?:^[ \t]*\|.+\|[ \t]*\n?)+)",
    re.MULTILINE,
)
_QQ_HEADING_RE = re.compile(r"^(#{1,6})\s+(.+)$", re.MULTILINE)
_QQ_BOLD_RE = re.compile(r"\*\*(.+?)\*\*")
_QQ_BOLD2_RE = re.compile(r"__(.+?)__")
_QQ_ITALIC_RE = re.compile(r"(?<!\*)\*(?!\*)(.+?)(?<!\*)\*(?!\*)")
_QQ_STRIKE_RE = re.compile(r"~~(.+?)~~")
_QQ_INLINE_CODE_RE = re.compile(r"`([^`]+)`")
_QQ_INLINE_LATEX_RE = re.compile(r"\$(.+?)\$")

_QQ_LATEX_REPLACEMENTS = (
    (r"\sqrt", "sqrt"),
    (r"\times", "×"),
    (r"\cdot", "·"),
    (r"\leq", "≤"),
...
def _strip_plain_markdown(text: str) -> str:
    text = _QQ_BOLD_RE.sub(r"\1", text)
    text = _QQ_BOLD2_RE.sub(r"\1", text)
    text = _QQ_ITALIC_RE.sub(r"\1", text)
    text = _QQ_STRIKE_RE.sub(r"\1", text)
    text = _QQ_INLINE_CODE_RE.sub(r"\1", text)
    text = _QQ_INLINE_LATEX_RE.sub(r"\1", text)
    for source, target in _QQ_LATEX_REPLACEMENTS:
        text = text.replace(source, target)
    return text


def _render_plain_table(table_text: str) -> str:
    lines = [line.strip() for line in table_text.strip().splitlines() if line.strip()]
    if len(lines) < 3:
        return _strip_plain_markdown(table_text.strip())

    def split_row(line: str) -> list[str]:
        return [_strip_plain_markdown(cell.strip()) for cell in line.strip("|").split("|")]
...
def _render_plain_content(content: str) -> str:
    rendered: list[str] = []
    last_end = 0
    for match in _QQ_TABLE_RE.finditer(content or ""):
        before = (content or "")[last_end:match.start()]
        if before:
            rendered.append(_strip_plain_markdown(before))
        rendered.append(_render_plain_table(match.group(1)))
        last_end = match.end()
    rendered.append(_strip_plain_markdown((content or "")[last_end:]))
    merged = "".join(rendered).strip()
    merged = _QQ_HEADING_RE.sub(lambda m: f"【{_strip_plain_markdown(m.group(2).strip())}】", merged)
    merged = re.sub(r"\n{3,}", "\n\n", merged)
    return merged
...
def _make_bot_class(channel: QQChannel) -> type[botpy.Client]:
    ...
    class _Bot(botpy.Client):
        def __init__(self):
            # Disable botpy's file log — mathclaw uses loguru; default "botpy.log" fails on read-only fs
...
class QQConfig(Base):
    ...
    # Optional: directory to save inbound attachments. If empty, use mathclaw get_media_dir("qq").
    media_dir: str = ""
...
class QQChannel(BaseChannel):
    ...
    def _init_media_root(self) -> Path:
        ...
        if self.config.media_dir:
            ...
            try:
                ...
                root = Path.home() / ".mathclaw" / "media" / "qq"
        else:
            root = Path.home() / ".mathclaw" / "media" / "qq"
    ...
    async def _send_text_only(
        ...
        if use_markdown:
            ...
            payload["content"] = render_plain_reply(content)
    ...
    async def _on_message(self, data: C2CMessage | GroupMessage, is_group: bool = False) -> None:
        ...
        media_paths, attachment_lines, att_meta = await self._handle_attachments(attachments)

        if attachment_lines:
            attachment_block = "\n".join(attachment_lines)
            content = f"{content}\n{attachment_block}".strip() if content else attachment_block
    ...
    async def _handle_attachments(
        ...
        """Extract, download, and normalize QQ attachments for shared attachment rules."""
        media_paths: list[str] = []
        attachment_lines: list[str] = []
        att_meta: list[dict[str, Any]] = []

        if not attachments:
            return media_paths, attachment_lines, att_meta
        ...
        for att in attachments:
            ...
            if local_path:
                ...
                saved_name = filename or os.path.basename(local_path)
                prefix = "[image:" if _is_image_name(saved_name) else "[file:"
                attachment_lines.append(f"{prefix} {saved_name}]")
            else:
                shown_name = filename or url
                prefix = "[image:" if _is_image_name(shown_name) else "[file:"
                attachment_lines.append(f"{prefix} {shown_name}: download failed]")
